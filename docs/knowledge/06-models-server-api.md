...
- **CLI argument building** (`server/config.py`) from settings:
  `--ctx-size` (default 4096), `--n-gpu-layers` (default 99; offload layers to
  GPU), `--threads` (-1 = auto), `--batch-size` / `--ubatch-size` (512),
  `--flash-attn` (on), `--mmap` (on), `--mlock` (off), `--numa`, plus the model
  path.
...
| `ModelProfiles` (`models/profiles.py`) | Saves/loads per-model configuration presets (sampling + server args). |
